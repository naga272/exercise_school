...
def push_on(argc: int, argv: list):
    ...
    for element in session.query(ContravvenzioneDB):
        auto        = session.query(AutoDB).filter_by(targa = element.targa)
        guidatore   = session.query(GuidatoreDB).filter_by(cod_fisc = element.cod_fisc).first()
        vigile      = session.query(VigileDB).filter_by(cod_fisc = element.matricola).first()
        result = requests.get(f"rapi/{vigile.matricola}_{auto.targa}_{guidatore.cod_fisc}_{element.luogo}_{element.dats}_{element.tipo_infrazione}_{element.importo}")
    return ''
...
def contravvenzione(argc: int, argv: list) -> str:
    ...
    if argc != ContravvenzioneDB().get_num_campi():  #  e' escluso il comando push_in_db
        ...
        Sono richiesti {ContravvenzioneDB().get_num_campi()} argomenti: matricola_vigile, targa, luogo, datetime, tipo_infrazione, importo.
        """
    ...
    try:
        ...
        luogo           = str(argv[3])
        datetime        = str(argv[4])
        tipo_infrazione = str(argv[5])
        importo         = float(argv[6])
