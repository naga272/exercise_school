...
def main(argc:int, argv:list) -> int:
    ...
    with open(argv[2], "w") as f_out:
        ...
        with open("./tasks.sql", "w") as fsql:
            ...
            for _, row in df_filtrato.iterrows(): # eseguo un unico ciclo per elaborare i dati per il file f_out e fsql
                ...
                f_out.write(f"{','.join(str(value) for value in row.values)}\n") # Converto la riga in una stringa separata da virgole

                fsql.write(f"INSERT INTO t_tasks ({insert_sql_column_name})\n")
                fsql.write(f"VALUES ({get_row(row)});\n\n")

    # Sostituisci gli spazi nei nomi delle colonne
    df_filtrato.columns = df_filtrato.columns.str.replace(' ', '_')
